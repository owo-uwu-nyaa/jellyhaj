...
use jellyhaj_core::{CommandMapper, context::TuiContext, keybinds::InspectCommand, render::{Erased, make_new_erased}, state::Navigation};
...
pub fn render_inspect(cx: TuiContext) -> Erased{
    let widget =
        OuterWidget::<Name,_>::new(KeybindWidget::new(
            InspectWidget::default() ,cx.config.keybinds.inspect.clone(), Mapper));
